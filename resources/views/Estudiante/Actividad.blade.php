...
<div class="container mt-4">
    <a href="{{ route('Curso', $actividades->subtema->tema->curso_id)}}" class="btn btn-primary mb-4">
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            ...
            @if (auth()->user()->hasRole('Estudiante'))
                            ...
                            <div class="card mb-3">
                                <div class="card-body">
                                    ...
                                    @if (!$actividades->fecha_limite || now() <= \Carbon\Carbon::parse($actividades->fecha_limite))
                                        <a href="{{ route('quitarEntrega', $entrega->id) }}" class="btn btn-sm btn-danger ml-2">Quitar Entrega</a>
                                    @endif
                                </div>
                            </div>
                        @endif
                ...
                @else
                    <form action="{{ route('subirArchivo', $actividades->id) }}" method="POST" enctype="multipart/form-data" class="mt-4">
                        @csrf
