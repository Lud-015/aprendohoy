                ...
                <div class="card-body">
                    ...
                    @if (auth()->user()->hasRole('Docente') && $cursos->docente_id == auth()->user()->id)
                        <div class="mt-2">
                            ...
                            {{-- <form action="{{ route('eliminarRecursosSubtema', $recurso->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                        ...
                                        <form method="POST" action="{{ route('editarRecursosSubtemaPost', $recurso->id) }}"
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <small class="form-text text-muted">
                                                    Formatos permitidos: Imágenes, documentos, audio, video y archivos comprimidos (máx. 10MB)
                                                </small>
                                            </div>
                                            <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fileInputs.forEach(input => {
        input.addEventListener('change', function() {
            ...
            if (file) {
                ...
                // Solo validar tipo de archivo si el input tiene el atributo accept="image/*"
                if (this.accept === 'image/*') {
                    const allowedTypes = ['image/jpeg', 'image/png', 'image/gif'];
                    if (!allowedTypes.includes(file.type)) {
                        alert('Tipo de archivo no permitido. Solo se permiten archivos JPG, PNG y GIF.');
                        this.value = '';
                        return;
                    }
                }
    ...
    if (resourceSelect) {
        resourceSelect.addEventListener('change', function() {
            ...
            const fileUpload = document.querySelector('input[name="archivo"]');

            if (!fileUpload) return;

            // Configurar el accept según el tipo de recurso
            switch(selectedType) {
                case 'word':
                ...
                case 'imagen':
                    ...
                    break;
                case 'video':
                    fileUpload.accept = '.mp4,.webm,.ogg';
                    break;
                case 'audio':
                    fileUpload.accept = '.mp3,.wav,.ogg';
                    break;
                case 'archivos-adjuntos':
                    fileUpload.accept = '*.*';
                    break;
                default:
                    // Para tipos basados en enlaces (youtube, forms, etc.)
                    fileUpload.accept = '';
                    break;
            }

            // Mostrar/ocultar el campo de archivo según el tipo
            if (['youtube', 'forms', 'drive', 'kahoot', 'canva', 'zoom', 'meet', 'teams', 'enlace'].includes(selectedType)) {
                fileUpload.parentElement.style.display = 'none';
            } else {
                fileUpload.parentElement.style.display = 'block';
            }
        });
    }
});
