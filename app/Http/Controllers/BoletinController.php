    ...
    public function guardar_boletin(Request $request)
{
    ...
    // Verificar si el estudiante ya tiene un boletín
    $boletin = Boletin::where('inscripcion_id', $estudianteId)->first();

    // Si no existe un boletín para el estudiante, crea uno nuevo
    if (!$boletin) {
        $boletin = new Boletin;
...
public function enviarBoletin(Request $request, $id)
{
    ...
    if ($inscritos) {
        // Capturar el contenido HTML de la vista
        ...
        // Envía el correo utilizando SwiftMailer
